...
const addForgotPasswordEmailTemplates_ru = async db => {
	...
	if (emailTemplatesNotExists) {
		await db.collection('emailTemplates').insertOne({
			...
			link: 'сброс пароля',
			subject: `Обновление пароля в `,
			body: `<div style="line-height: 30px">
			<div><b><h2>{{shop_name}}</h2></b><div>
			<div><b>Мы получили ваш запрос на сброс пароля, связанного с этим адресом электронной почты.</b><div>
			<div><b>Нажмите на следующую ссылку, чтобы сбросить пароль:</b><br />{{forgot_password_link}}<div>
			<br />
			<div><b>Если у вас есть какие-либо вопросы, пожалуйста, свяжитесь с нашей службой поддержки.</b></div>
			<br />
			<div><b>С наилучшими пожеланиями</b></div>
			<br />
...
const addRegisterDoiEmailTemplates_ru = async db => {
	...
	if (emailTemplatesNotExists) {
		await db.collection('emailTemplates').insertOne({
			...
			link: 'зарегистрируйтесь сейчас',
			subject: `Новая регистрация на `,
			body: `<div style="line-height: 30px">
			<div><b><h2>{{shop_name}}</h2></b><div>
			<div><b>Вы собираетесь открыть новую учетную запись в {{shop_name}}.</b><div>
			<div><b>Нажмите на следующую ссылку, чтобы подтвердить новый логин и открыть свой аккаунт:</b><br />
			{{register_doi_link}}
			<br />
			<div><b>Если у вас есть какие-либо вопросы, пожалуйста, свяжитесь с нашей службой поддержки.</b></div>
			<br />
